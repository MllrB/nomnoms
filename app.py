...
def recipe_init():
    ...
    # add key value pairs for recipe filters and allow for
    # additions/deletions to filter keywords at db level
...
@app.route('/home')
def home():
    # Route for returning to homepage and initialising
    # the session cookie used to add recipes
...
def browse_recipes():
    ...
    categories = mongo.db.optionalTypes.find_one(
                {'name': 'recipe_type'})['values']
    recipe_info = mongo.db.optionalTypes.find_one({'name': 'recipe_info'})
...
def filter_recipes():
    ...
    if request.method == 'POST':
        ...
        if no_of_docs > 0:
            ...
            categories = mongo.db.optionalTypes.find_one(
                        {'name': 'recipe_type'})['values']
            recipe_info = mongo.db.optionalTypes.find_one(
                                  {'name': 'recipe_info'})
            return render_template('browse_recipes.html',
                                   recipes=recipes,
                                   categories=categories,
...
def search_recipes():
    ...
    # matches are found from title, ingredient name,
    # categories, dietary information and meal type
    recipes = mongo.db.scrambledeggs.find()
    if request.method == 'POST':
        ...
        return render_template('browse_recipes.html',
                               ...
                               no_of_results=len(recipes_found),
                               categories=categories,
                               recipe_info=recipe_info)
...
def create_recipe():
    ...
    measurements_list = mongo.db.optionalTypes.find_one(
                        {'name': 'measurements'})['values']
    categories = mongo.db.optionalTypes.find_one(
                        {'name': 'recipe_type'})['values']
    recipe_info = mongo.db.optionalTypes.find_one(
                        {'name': 'recipe_info'})
...
def add_recipe():
    # uses a session cookie to allow users to dynamically add
    # ingredients and steps before saving the recipe to the database
    if request.method == 'POST':
        form_values = request.form.to_dict()
        ...
        if 'remove_ingredient' in request.form:
            ...
            new_ingredient = {
                             'ingredient_name': form_values['ingredient_name'],
            ...
            session['recipe']['steps'] = add_step(session['recipe'],
                                                  new_step_value)
        elif 'save_recipe' in request.form:
            session['recipe']['category'] = form_values['category']
            # remove unnecessary fields and values before further processing
            del form_values['ingredient_name'],
            form_values['quantity'], form_values['measurement']
...
def edit_recipe():
    # renders edit_recipe.html which asks for user's
    # name and pin protection before proceeding
...
def find_recipe_to_edit():
    ...
    if request.method == 'POST':
        ...
        recipes = mongo.db.scrambledeggs.find({'pin': pincode['pin'],
                                              'owner': owner})
        no_of_docs = mongo.db.scrambledeggs.count_documents(
                      {'pin': pincode['pin'], 'owner': owner})

        if no_of_docs > 0:
            return render_template('choose_recipe_to_edit.html',
                                   recipes=recipes)
        else:
            return render_template('edit_recipe.html',
                                   doc_not_found=True)
...
def update_recipe(recipe_id):
    ...
    if request.method == 'POST':
        ...
        if 'remove_ingredient' in request.form:
            ...
            ingredient_to_remove = request.form['remove_ingredient']
            new_ingredients = remove_ingredient(recipe, ingredient_to_remove)
            mongo.db.scrambledeggs.update_one(
            ...
            mongo.db.scrambledeggs.update_one(
                {'_id': ObjectId(recipe_id)}, {'$set': {'steps': new_steps}})
        elif 'save_recipe' in request.form:
            ...
            del form_values['ingredient_name'],
            form_values['quantity'],
            form_values['measurement']
            ...
            for form_key in form_values:
                ...
                for index, ingredient in enumerate(recipe['ingredients']):
                    if ingredient['ingredient_id'] in form_key:
                        new_key = form_key.split(
                                  '_' + ingredient['ingredient_id'])
            ...
            mongo.db.scrambledeggs.replace_one(
                {'_id': ObjectId(recipe_id)}, recipe)

            recipes = mongo.db.scrambledeggs.find(
                {'pin': recipe['pin'], 'owner': recipe['owner']})

            return render_template('choose_recipe_to_edit.html',
                                   recipes=recipes)

    # reloads the current recipe for further editing
    # if the save recipe button has not yet been clicked
    return redirect(url_for('recipe_to_update',
                            recipe_id=recipe_id))


@app.route('/remove_recipe')
def remove_recipe():
    # renders remove_recipe.html which asks for user's
    # name and pin protection before proceeding
...
def find_recipes_for_removal():
    ...
    if request.method == 'POST':
        ...
        if no_of_docs > 0:
            return render_template('choose_recipe_to_remove.html',
                                   recipes=recipes)
        else:
            return render_template('remove_recipe.html',
                                   doc_not_found=True)
...
def delete_recipe(recipe_id):
    ...
    if request.method == 'POST':
        ...
        # If the user clicks the browsers back button they will
        # see the deleted documents due to the previous page being
        # stored in the browser cache. In this case, this check
        # prevents the app from crashing if the user tries to
        # delete an already deleted document by changing the
        # message parameter passed to recipe_removed.html
        no_of_docs = mongo.db.scrambledeggs.count_documents(
                                {"_id": ObjectId(recipe_id)})
        if no_of_docs > 0:
            recipe = mongo.db.scrambledeggs.find_one(
                        {"_id": ObjectId(recipe_id)})

            title = recipe['title']
            removed_message = f'Your recipe \"{title}\" has been deleted'
            mongo.db.scrambledeggs.remove(
                                    {"_id": ObjectId(recipe_id)}, {True})

            return render_template('recipe_removed.html',
